...
class idnet(nn.Module):
    def __init__(self, input_size=224, se=False, avgdown=False, width_mult=1., merge=False):
        ...
        self.branch1 = [conv_bn(3, input_channel, 2)]
        self.branch2 = [conv_bn(3, input_channel, 2)]
        ...
        self.branch1.append(block(input_channel, output_channel, 2, expand_ratio=1, downsample=downsample))
        self.branch2.append(block(input_channel, output_channel, 2, expand_ratio=1, downsample=downsample))

        input_channel = output_channel
        if self.se:
            self.branch1.append(SELayer(input_channel))
            self.branch2.append(SELayer(input_channel))

        self.branch1 = nn.Sequential(*self.branch1)
        self.branch2 = nn.Sequential(*self.branch2)

        input_channel = input_channel * 2 if merge else input_channel
...
def IDNetm():
    model = idnet(se=True, avgdown=True, merge=True)
    return model
